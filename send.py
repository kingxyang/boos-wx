...
link = "https://kingxyang.github.io/"
...
def get_pic():
    try:
        ...
        pic_url = "https://api.btstu.cn/sjbz/api.php?format=json&lx=suiji"
        r = requests.get(pic_url).json()
        return r["imgurl"]
    except Exception as e:
        print("获取随机图片数据出错,请稍后重试", e)
        return None
...
def get_weather(city_name):
    try:
        ...
        print(f"获取{city_name}和风天气数据出错,请检查填写是否正确或者稍后重试", e)
        return None
...
def main():
    if corpid and corpsecret and agentid:
        ...
        if res["errcode"] == 0:
            ...
            print("企业微信消息发送失败: 请检查填写是否正确或者稍后重试 " + str(res))
            return 0
    else:
        print("企业微信应用配置缺失")
        return 0
